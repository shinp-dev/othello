...
test("scheduled maintenance queues expired accounts before processing", async t => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (String(url).includes("run_legacy_match_maintenance_v1")) return Response.json([{
      terminalized_matches: 0,
      deleted_signals: 0,
      deleted_queue_rows: 0,
    }]);
  ...
  assert.deepEqual(calls.map(call => call.url), [
    ...
    `${env.SUPABASE_URL}/rest/v1/rpc/run_legacy_match_maintenance_v1`,
  ...
  const legacyMaintenance = calls.find(call => call.url.includes("run_legacy_match_maintenance_v1"));
  assert.equal(legacyMaintenance.method, "POST");
  assert.deepEqual(JSON.parse(legacyMaintenance.body), { p_limit: 100 });
});
...
test("scheduled tasks are isolated but any maintenance failure rejects the cron work", async t => {
  ...
  assert.ok(calls.some(url => url.includes("run_legacy_match_maintenance_v1")));
  assert.ok(calls.some(url => url.includes("account_deletion_requests?")));
});

test("legacy cleanup failure rejects cron work without suppressing existing maintenance", async t => {
  const originalFetch = globalThis.fetch;
  const originalError = console.error;
  const calls = [];
  globalThis.fetch = async url => {
    calls.push(String(url));
    if (String(url).includes("run_legacy_match_maintenance_v1")) {
