-- Canonical move serialization permits zero-ply resignation/timeout/disconnect results,
-- while NORMAL results still require at least one move token.

alter table public.match_submissions drop constraint if exists match_submissions_canonical_moves_format;
alter table public.game_records drop constraint if exists game_records_canonical_moves_format;
alter table public.match_submissions add constraint match_submissions_canonical_moves_format
  check (
    canonical_moves is null
    or canonical_moves ~ '^((--|[a-h][1-8])+)$'
...
begin
  ...
  if p_canonical_moves is null or char_length(p_canonical_moves) > 240
     or (p_canonical_moves = '' and p_finish_reason = 'NORMAL')
     or (p_canonical_moves <> '' and p_canonical_moves !~ '^((--|[a-h][1-8])+)$') then
    raise exception 'invalid canonical moves';
  end if;
...
begin
  ...
  if black_submission.match_id is null or white_submission.match_id is null then
    update public.matches set server_status = 'PENDING_RESULT', result_expires_at = coalesce(result_expires_at, now() + interval '30 days') where id = p_match_id;
    update public.active_match_participants set expires_at = (select result_expires_at from public.matches where id = p_match_id) where match_id = p_match_id;
...
comment on function public.cleanup_terminal_matches() is
  'Maintenance-only retention cleanup. Run from Supabase Cron/pg_cron, not matchmaking hot path.';
