...
$releaseRpcPatterns = @(
    ...
    '(?s)rpcName\s*=\s*"ack_match_started_v2".{0,300}?releaseStartStateRpc\(context\).{0,300}?client\.postgrest\.rpc\(',
    '(?s)rpcName\s*=\s*"get_release_match_state_v2".{0,300}?releaseStartStateRpc\(context\).{0,300}?client\.postgrest\.rpc\(',
    '(?s)rpc\("abandon_match_v2".{0,160}?decodeAs<String>\(\)',
    '(?s)rpc\(\s*"submit_match_result_v2".{0,800}?decodeList<ReleaseResultRow>\(\)\.single\(\)',
    '(?s)rpcName\s*=\s*"resume_match_v2".{0,300}?releaseStartStateRpc\(context\).{0,300}?client\.postgrest\.rpc\(',
    '(?s)response\.decodeList<ReleaseMatchStateRow>\(\).{0,1200}?rows\.single\(\)',
