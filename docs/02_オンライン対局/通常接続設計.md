...
| 結果提出 | サーバーRPC | 永続化へ入る用途を絞った認証済み入口。正常終了では双方から提出された確認材料（確認材料）を待つ |
...
| 対象接続世代（期待するepoch） | ACKやresume リクエストが「どのepochを対象にしているか」をサーバーへ伝える番号。古いリクエストを新しいepochへ適用しないための識別札になる。 |
...
| プロトコル 2 | リリース-hardening クライアントが使う、サーバー-正式な 結果とepoch-aware connection/reconnectを持つ現行オンライン 対局 プロトコル。 |
| ロール / disc | 盤上のBLACK / WHITE。WebRTCの初回signaling ロールにも使い、BLACKがOFFER、WHITEがANSWERを担当する。 |
...
通知またはHTTP応答が失われても、同じ`requestId`での定期確認、`claim_active_match_v2`、初期SELECTと上限付きの状態照合（reconciliation）が有効な割り当てやsignalingを回収します。Realtimeだけを唯一の配信保証にはしていません。

## 6. 通常接続のクライアント状態

Android側の論理状態は次のように進みます。実装上は`MatchmakingController`が`IDLE`、`WAITING`、`SIGNALING`を担当し、割り当てが`OnlineSessionViewModel`へ渡された後、`OnlineMatchController`が`P2P_CONNECTED`以降を担当します。図はその引き継ぎ（引き継ぎ）を1本のセッションとして表しています。
...
プロトコル 2のサーバー persisted 状態は`matches.release_status`です。通常系で関係する主要遷移だけを抜き出すと次のとおりです。
...
購読開始時は既存行のSELECTとRealtimeを組み合わせ、その境界で通知を取り逃がす競合（race condition）に対して、順序付きSELECTを上限1回で行います。この状態照合（reconciliation）後もsignaling 購読自体は`WebRtcMatchCoordinator.close()`まで維持されます。通常の着手を運ぶためではなく、接続中の調整と後の再接続に備えるためです。サーバーが`ACTIVE`の間はRLS上、初回signaling 行は進行中 signaling対象ではありません。
...
1人目のACKだけでは`release_status`は`MATCHED`のままです。2 参加者のACKが同じ現在の接続世代（epoch）に揃ったときだけ、サーバーは`ACTIVE`へ遷移します。`OnlineMatchController`はACK 応答だけに依存せず`get_release_match_state_v2`でも正式状態を取得し、同じepochで`ACTIVE`かつboth ACKを確認してから`StartConfirmed`を適用し、`PLAYING`と時計開始へ進みます。
...
これは「TURNは常に有料だから使わない」という固定的な料金判断でも、結果のセキュリティ hardeningの欠陥でもありません。現在の製品における到達性（到達可能性）・可用性（可用性）とサーバー relay運用のトレードオフです。実ネットワークでの成功率は2台実機テストで確認し、TURNが必要かは利用環境と運用条件を含めて別途判断します。
...
正常終了 結果では、同じepochについて双方が同一の正規化済み着手履歴（canonical moves）と正常終了 確認材料を提出し、replayが終了済みまで成功した場合だけ確定経路へ進みます。片側だけなら`RESULT_PENDING`で待ち、確認材料が一致しなければ`DISPUTED`となりレートを変更しません。

確定時は1トランザクションで、`match_results_v2`、変更不能なな`game_records`、双方の`rating_history`、`ratings`、`user_game_records`を整合させます。既存Research 収集を起動するlegacy `server_status=CONFIRMED`への更新は、それらが揃った後のトランザクション末尾で行われます。Research参加同意など別の条件はResearch側の設計に従います。
...
- Androidからofficial レート / peak / verified 結果を直接UPDATEできない
...
P2Pでサーバーが毎手を観測しない以上、2つのcolluding アカウントが同じ合法棋譜を作るresidual リスクまでは消えません。一方、illegal transcript、arbitrary winner、syntactically validなfake hashをそのまま公式結果にする経路はサーバー replayで閉じています。コスト境界とtrust 境界の両方を明示した設計です。

## 17. コストを考慮したarchitecture
...
通常の1手はDataChannel上の着手 コマンドと小さな相手端末 ACKで完結し、Supabase リクエストを発生させません。サーバー処理はキュー、signaling、開始、終局などの境界へ集中します。これにより、対局時間中の時計 tickや手数が、そのままRealtime使用量やDB 書き込み数にはなりません。
...
| サーバー-正式な 確定処理 | レート、GameRecord、Researchのソースの整合性を守る | replay、双方の 確認材料、トランザクション、タイムアウト 状態照合（reconciliation）が必要 |
| stable リクエスト / 通知＋定期確認 | lost 応答や通知 遅延から回復できる | クライアントとSQLのidempotency、lease、ロックが必要 |

この複雑さを受け入れた理由は、高頻度trafficをP2Pへ移しても、開始・結果・永続データの信頼境界（trust 境界）をサーバーに残すためです。P2P特有の非対称な障害観測がReconnect プロトコルを複雑にした経緯は、[再接続設計](再接続設計.md)で詳しく扱います。
...
6. **再試行は状態を認識し、冪等にする。** stable リクエストID、コマンド ID、期待するepochを持たせ、重複deliveryや応答喪失を二重処理へ変えない。
...
この設計は、すべてのネットワークでP2P接続できることや、すべての不具合がないことを証明するものではありません。現在のコードが守る責任分界と不変条件（不変条件）を示し、STUN-onlyの到達性、Android WebRTCの実挙動、実ネットワーク上の順序（順序）は2台実機テストで確認する対象として残します。
