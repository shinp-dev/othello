...
deletion boundary introduced by PR 2E. Do not edit or re-run an already
applied migration. Inspect the result after each file.

## Worker configuration

The trusted Cloudflare Worker is `othello-admin`. Non-secret vars are in
`cloudflare-admin/wrangler.toml`. Set these only in the Worker secret store:
...
The Worker Cron runs every 10 minutes and independently attempts:

- pending account deletion processing;
- validation of a bounded batch of captured games;
- one lease-protected aggregate generation build.

Each task is isolated with `Promise.allSettled`; a research failure does not
stop deletion processing or live matchmaking. A failed generation remains
non-published and the previous published generation stays active.
...
generation, BUILDING/FAILED generations, Worker errors, RPC errors, and the
current collection flag. Do not copy research subject IDs or raw moves into
public logs or support messages.
...
5. Correct data or code under a reviewed change, then run the aggregate
   endpoint again. The old published generation remains active until the new
   generation reaches `PUBLISHED` and the pointer switches atomically.
6. Re-run private ACL and public response checks.

Lease expiry and Worker retry are expected recovery paths. A poison source
...
- Roll back the Worker only if the previous version preserves service-only
  ACLs and account-unlink ordering.
