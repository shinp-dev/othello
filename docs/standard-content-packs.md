...
Standardガチャは無料で、端末のローカル日付ごとに3回まで引ける。
日付の境界は端末ローカル時刻の0:00とし、保存済み日付より未来の日付へ進んだ時だけ
回数をリセットする。端末時刻を過去へ戻しても回数は復活しない。
日次回数は認証ユーザー単位で端末内に保存し、アンインストール時には初期化される。
現時点では課金通貨や広告視聴による追加回数は提供しない。

抽選poolには未取得・取得済みを常に含めるため、図鑑未完成でも重複cardが出る。
NEW表示だけはstable `cardId` の取得済み状態から判定する。
